...
from reward_preprocess import get_dataset, convert_symbols_to_inputs, get_vocab_size
...
config = RewardTransformerConfig(vocab_size=get_vocab_size(),
...
for epoch in trange(1, int(args.epochs)+1, desc="Epoch"):
    ...
    if epoch % 20 == 0:
        torch.save(model, "pretrain_reward_models/epoch_%s_pretrain_reward_model.pkl"%epoch)
